...
// variable arguments method call
void insertINVOKESTATIC_MULTIARG(char *className, ParType returnType, int argsnum, ...)
{
	char instruction[MAX_INST_LEN];
    va_list methodArgs;
	va_start(methodArgs,argsnum);
	char jArgDes[argsnum]; 
	jArgDes[0]='\0'; // empty string
	for(int i=0;i<argsnum;i++)
	{
		strcat( jArgDes,typeDescriptor(va_arg(methodArgs,int)) );
	}
	sprintf(instruction,"invokestatic %s(%s)%s",className,
	               jArgDes,typeDescriptor(returnType));
	addInstruction(instruction,NO_LABEL);
}
